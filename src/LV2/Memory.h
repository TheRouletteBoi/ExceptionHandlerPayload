...
{
    return STATIC_FN(&page_allocate, g_LibLV2.page_allocate_opd)(process, numBytes, pageID, pageSize, pageAddressOut);
}

static inline uint32_t page_free(lv2::process* process, void* pageAddress, uint64_t pageID)
{
    return STATIC_FN(&page_free, g_LibLV2.page_free_opd)(process, pageAddress, pageID);
}
...
{
    return STATIC_FN(&kmem_export_to_proc, g_LibLV2.kmem_export_to_proc_opd)(process, pageAddress, flags, pageVirtualAddressOut);
}

static inline uint32_t kmem_unexport_from_proc(lv2::process* process, uint64_t pageVirtualAddress)
{
    return STATIC_FN(&kmem_unexport_from_proc, g_LibLV2.kmem_unexport_from_proc_opd)(process, pageVirtualAddress);
}
